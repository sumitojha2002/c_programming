...
void sum(int a[30][30],int b[30][30],int row,int col);
int i,j;

int main(){
	int arrayA[30][30],arrayB[30][30],row,col;
	printf("Enter the size of the row and col: ");
	scanf("%d %d",&row,&col);
	input(arrayA,row,col);
	input(arrayB,row,col);
	display(arrayA,row,col);
	display(arrayB,row,col);
	sum(arrayA,arrayB,row,col);
  return 0;
}
